...
class ComposeAsymmetrical:
    ...
    def __call__(self, pc, features, img, pairing_points, pairing_images, pairing_labels=None, superpixels=None):
        for transform in self.transforms:
            pc, features, img, pairing_points, pairing_images, pairing_labels, superpixels = transform(
                pc, features, img, pairing_points, pairing_images, pairing_labels, superpixels
            )
        if superpixels is None:
            return pc, features, img, pairing_points, pairing_images, pairing_labels
        return pc, features, img, pairing_points, pairing_images, pairing_labels, superpixels
...
class ResizedCrop:
    ...
    def __call__(self, pc, features, images, pairing_points, pairing_images, pairing_labels, superpixels):
        imgs = torch.empty(
        ...
        if self.crop_center:
            ...
            for id, img in enumerate(images):
                ...
                pairing_labels_out = np.concatenate(
                    (pairing_labels_out, pairing_labels[mask][valid_indexes])
                )                
        if superpixels is None:
            return pc, features, imgs, pairing_points_out, pairing_images_out, pairing_labels_out, superpixels
        return pc, features, imgs, pairing_points_out, pairing_images_out, pairing_labels_out, sps
...
class FlipHorizontal:
    ...
    def __call__(self, pc, features, images, pairing_points, pairing_images, pairing_labels, superpixels):
        ...
        return pc, features, images, pairing_points, pairing_images, pairing_labels, superpixels
...
class DropCuboids:
    ...
    def __call__(self, pc, features, images, pairing_points, pairing_images, pairing_labels, superpixels):
        ...
        pairing_labels_out = pairing_labels[mask]
