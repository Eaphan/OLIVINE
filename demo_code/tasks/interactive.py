...
def interactive_infer_image(model, image, tasks, sp_path=None, refimg=None, reftxt=None, audio_pth=None, video_pth=None):
    ...
    if 'Panoptic' in tasks:
        ...
        res = demo.get_image()
        # import pdb; pdb.set_trace()
        # cv2.imwrite('./seem_sample_kitti.png', res)
        im = Image.fromarray(pano_seg.detach().cpu().numpy().astype(np.uint8))
        im.save(sp_path)
        with open(sp_path.replace('png', 'json'),"w") as f:
    ...
    res = torch.zeros(pred_masks_pos[0].shape).long()
    for idx, mask in enumerate(pred_masks_pos):
        res[mask==1] = pred_class[idx]

    im = Image.fromarray(res.detach().cpu().numpy().astype(np.uint8))
    im.save(sp_path)
    # with open(sp_path.replace('png', 'json'),"w") as f:
    #     json.dump(pano_seg_info,f)
