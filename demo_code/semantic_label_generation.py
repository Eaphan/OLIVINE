...
def compute_seem(cam_token):
    ...
    sp_path = (args.sp_folder + cam["filename"]).replace('.jpg', ".png")
    os.makedirs(os.path.dirname(sp_path), exist_ok=True)
    if os.path.exists(sp_path): return
    image = Image.open(os.path.join(nusc.dataroot, cam["filename"]))
    image = {'image':image, 'mask':None}
    with torch.autocast(device_type='cuda', dtype=torch.float16):
        interactive_infer_image(model,  image, task, sp_path=(args.sp_folder + cam["filename"]).replace('.jpg', ".png"), reftxt=COCO_PANOPTIC_CLASSES + ["background"])
