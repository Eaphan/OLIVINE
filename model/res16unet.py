...
class Res16UNetBase(ResNetBase):
    ...
    def forward(self, x):
        ...
        if "is_pretraining" in self.config and self.config["is_pretraining"]:
            out_class_level = self.final_class_level(out_b)
            if self.normalize_feature:
                return SparseTensor(
                    ...
                    out_class_level.F / torch.norm(out_class_level.F, p=2, dim=1, keepdim=True),
                    coordinate_map_key=out_class_level.coordinate_map_key,
                    coordinate_manager=out_class_level.coordinate_manager,
                )
            else:
                return out, out_class_level
        else:
            if self.normalize_feature:
                return SparseTensor(
                    out.F / torch.norm(out.F, p=2, dim=1, keepdim=True),
