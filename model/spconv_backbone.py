...
class VoxelNet(VoxelBackBone8x):
    def __init__(self, in_channels, out_channels, config, D=3):
        ...
        self.config = config
        if "is_pretraining" in config and config["is_pretraining"]:
            self.final_class_level = spconv.SparseConv3d(
                128,
                out_channels // 1,
                1,
                stride=1,
    ...
    def forward(self, voxels, coordinates):
        ...
        sp_tensor_b = super(VoxelNet, self).forward(sp_tensor)
        sp_tensor = self.final(sp_tensor_b)
        ...
        if "is_pretraining" in self.config and self.config["is_pretraining"]:
            sp_tensor_class_level = self.final_class_level(sp_tensor_b)
            sp_tensor_class_level = sp_tensor_class_level.replace_feature(nn.functional.normalize(sp_tensor_class_level.features, dim=1))
            sp_tensor_class_level = self.height_compression(sp_tensor_class_level)
            return sp_tensor, sp_tensor_class_level
        else:
            return sp_tensor
